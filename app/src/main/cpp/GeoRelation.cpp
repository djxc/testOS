...
bool GeoRelation:: equal_point(POINT p1,POINT p2)           // 判断两个点是否重合
{
...
*******************************************************************************/
bool _online(LINESEG l,POINT p)
{
    return( (multiply(l.e,p,l.s)==0) &&( ( (p.x-l.s.x)*(p.x-l.e.x)<=0 )&&( (p.y-l.s.y)*(p.y-l.e.y)<=0 ) ) );
}

bool GeoRelation::online(LINESEG l,POINT p)
{
    return _online(l, p);
}
...
{
    return( (fmax(u.s.x,u.e.x)>=fmin(v.s.x,v.e.x))&&                     //排斥实验
            (fmax(v.s.x,v.e.x)>=fmin(u.s.x,u.e.x))&&
            (fmax(u.s.y,u.e.y)>=fmin(v.s.y,v.e.y))&&
            (fmax(v.s.y,v.e.y)>=fmin(u.s.y,u.e.y))&&
...
int insidepolygon(int vcount,POINT Polygon[],POINT q)
{
    int c=0,i,n;
    LINESEG l1,l2;
    bool bintersect_a,b_online1,b_online2,b_online3;
    double r1,r2;
    ...
    for (i=0;i<vcount;i++)
    {
        ...
        if ( (bintersect_a=intersect_A(l1,l2))|| // 相交且不在端点
             ( (b_online1=_online(l1,Polygon[(i+1)%n]))&& // 第二个端点在射线上
               ( (!(b_online2=_online(l1,Polygon[(i+2)%n])))&& /* 前一个端点和后一个端点在射线两侧 */
                 ((r1=multiply(Polygon[i],Polygon[(i+1)%n],l1.s)*multiply(Polygon[(i+1)%n],Polygon[(i+2)%n],l1.s))>0) ||
                 (b_online3=_online(l1,Polygon[(i+2)%n]))&&     /* 下一条边是水平线，前一个端点和后一个端点在射线两侧  */
...
bool LinesegInsidePolygon(int vcount,POINT polygon[],LINESEG l)
{
    ...
    for(i=0;i<vcount;i++)
    {
        ...
        if(_online(s,l.s)) //线段l的起始端点在线段s上
            PointSet[top++]=l.s;
        else if(_online(s,l.e)) //线段l的终止端点在线段s上
            PointSet[top++]=l.e;
        else
        {
            if(_online(l,s.s)) //线段s的起始端点在线段l上
                PointSet[top++]=s.s;
            else if(_online(l,s.e)) // 线段s的终止端点在线段l上
                PointSet[top++]=s.e;
            else
            {
